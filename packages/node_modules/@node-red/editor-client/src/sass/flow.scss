...
.red-ui-flow-node {
    ...
    cursor: move;
    stroke-width: 1;
	filter: drop-shadow(7px 0 3px rgba(0,0,0,.5));
}
...
.red-ui-flow-node-error {
	...
	filter: drop-shadow(0 0 2px adjust-hue($node-status-error-border, 10))
}
.red-ui-flow-node-changed {
    ...
    stroke-linejoin: round;
    stroke-linecap: round;
	//filter: drop-shadow(0 0 3px #006eff)
	filter: drop-shadow(0 0 2px lighten(adjust-hue($node-status-changed-background, -20), 20%))
}
