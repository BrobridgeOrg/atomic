...
module.exports = function(RED) {
    ...
    RED.nodes.registerType("mongodb-aleph-config", MongoNode,{
        credentials: {
            user: {type:"text"},
                ...
                if (err) {
				    ...
				    if (done) {
                            ...
                            if (node.payonly) {
                                ...
                                coll.insertOne(msg.payload, function(err, item) {
                                    if (err) {
                                        node.error(err, msg);
                                        return;
                                    }
    ...
    RED.nodes.registerType("mongodb-aleph-out",MongoOutNode);
    ...
    function MongoInNode(n) {
        ...
        var connectToDB = function() {
			...
			*/
			    const aggCursor = coll.aggregate(msg.payload);
			    aggCursor.toArray(function(cursorError, cursorDocs){
				    if (cursorError){
					node.error(cursorError);
				    }else {
					msg.payload = cursorDocs;
    ...
    RED.nodes.registerType("mongodb-aleph-in",MongoInNode);
}
